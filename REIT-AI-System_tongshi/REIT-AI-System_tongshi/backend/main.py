...
# 登录鉴权中间件：accounts.json 存在时全站需登录（登录页/登录接口/健康检查除外）。
# 没有 accounts.json（本地开发）则整站开放，不受影响。
_AUTH_PUBLIC = {"/login", "/api/auth/login", "/api/health", "/favicon.ico"}
...
async def _require_login(request, call_next):
    ...
    if path in _AUTH_PUBLIC:
        return await call_next(request)
    if auth_module.current_user(request):
        return await call_next(request)
    # 未登录：API 请求回 401（前端据此跳登录），页面请求直接重定向到登录页
...
app.include_router(auth_module.router)   # /api/auth/login、/logout、/me
