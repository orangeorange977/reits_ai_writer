...
# ===== 生成取消（一键停止）=====
# 记录被请求取消的章节号；chat_with_tools 在轮/工具之间检查，置位则中止本次生成。
...
def project_overview() -> dict:
    ...
    - 名称/行业：取自摘要表；
    - 进度：已产出结构化 JSON（ch{n}.json 存在）的章节数 / 总章数；
    - 最近编辑时间：各章 JSON 与摘要表存档里最新的修改时间。"""
    import time
    saved = summary_service.get_summary_data() or {}

    def sv(label):
        for r in saved.get("summary_table", []):
    ...
    summary_filled = len(saved.get("summary_table") or []) > 0
    # 列表显示名：优先用项目组自定义的显示名，否则回退到摘要表"项目名称"
...
def _cell_span_attr(c):
    cs = int(c.get("colspan", 1) or 1)
    rs = int(c.get("rowspan", 1) or 1)
    return (f' colspan="{cs}"' if cs > 1 else "") + (f' rowspan="{rs}"' if rs > 1 else "")
    ...
    if headers:
        ...
        for h in headers:
            if isinstance(h, dict):
                ths += f"<th{_cell_span_attr(h)}>{_esc_html(h.get('text', ''))}</th>"
            else:
                ths += f"<th>{_esc_html(h)}</th>"
...
# 表格合并记号（写在 SKILL 单元格里，Kimi 照抄，代码在生成后确定性展开成 colspan/rowspan）：
_MERGE_UP = "^^"     # 该格并入正上方的格（rowspan）
_MERGE_LEFT = "<<"   # 该格并入左侧的格（colspan）
...
def _expand_matrix(mat: list) -> list:
    """把一个矩形单元格文本矩阵里的合并记号(^^ 向上 / << 向左)确定性地展开：
    被合并掉的格不再输出，合并锚点格带上 colspan/rowspan。返回逐行的单元格列表
    （无跨度→字符串；有跨度→{text,colspan,rowspan}）。"""
    R = len(mat)
    C = max((len(r) for r in mat), default=0)
    if R == 0 or C == 0:
        return [list(r) for r in mat]
    m = [list(r) + [""] * (C - len(r)) for r in mat]
    covered = [[False] * C for _ in range(R)]
    cs = [[1] * C for _ in range(R)]
    rs = [[1] * C for _ in range(R)]
    for r in range(R):
        for c in range(C):
            v = m[r][c]
            if v == _MERGE_LEFT:
                cc = c - 1
                while cc >= 0 and covered[r][cc]:
                    cc -= 1
                if cc >= 0:
                    cs[r][cc] += 1
                covered[r][c] = True
    ...
    for r in range(R):
        ...
        for c in range(C):
            if covered[r][c]:
                continue
            if cs[r][c] > 1 or rs[r][c] > 1:
                row.append({"text": m[r][c], "colspan": cs[r][c], "rowspan": rs[r][c]})
            else:
                row.append(m[r][c])
        out.append(row)
    return out
...
def _expand_grid_merges(g: dict) -> None:
    """就地展开一个 grid 块里的合并记号（^^ / <<）。没有记号则原样不动。
    表头单独按一行展开（只处理 << 横向合并），表体按整块展开（^^ 纵向 + << 横向）。"""
    headers = g.get("headers") or []
    rows = g.get("rows") or []
    all_texts = [_cell_text(h) for h in headers] + [_cell_text(c) for row in rows for c in (row or [])]
    if not any(t in (_MERGE_UP, _MERGE_LEFT) for t in all_texts):
        return
    if headers:
        hm = _expand_matrix([[_cell_text(h) for h in headers]])
        g["headers"] = hm[0] if hm else headers
    if rows:
        g["rows"] = _expand_matrix([[_cell_text(c) for c in (row or [])] for row in rows])


def _expand_all_grid_merges(sections: list) -> list:
    """对所有 section 里的 grid 块展开合并记号（在生成落盘前调用）。"""
    for s in sections or []:
        for b in _section_blocks(s):
            if isinstance(b, dict) and b.get("type") == "grid":
                _expand_grid_merges(b)
    return sections
            ...
            if self._table_kind == "grid" and self._grid is not None:
                if self._in_thead:
                    ...
                    for (t, cs, rs) in self._row:
                        if cs > 1 or rs > 1:
                            hdr.append({"text": t, "colspan": cs, "rowspan": rs})
                        else:
                            hdr.append(t)
                    self._grid["headers"] = hdr
                else:
                    cells = []
                    for (t, cs, rs) in self._row:
                        if cs > 1 or rs > 1:
...
def _format_saved_summary() -> str:
    """把用户已保存的摘要表/释义/项目总体情况格式化成文本，供拼进 prompt。"""
    ...
    return "\n\n".join([
        ...
        block("项目总体情况", saved.get("other_info")),
    ])
...
def _output_contract(chapter_title: str) -> str:
    return (
        ...
        "4.1 **表格里若出现合并记号 ^^（并入上一格）或 <<（并入左一格），必须原样照抄到对应单元格**，"
        "不要删除、不要改成空、不要自己去合并——系统会据此自动合并单元格。含记号的表按原样的行列数逐格输出。\n"
        "5. 值优先用“已保存的摘要表/释义/项目总体情况”里的真实值，其次 planning.md（引号标注要照抄的内容，"
...
def run_chapter(n: int, subtitles: list = None, materials_path: str = None,
    ...
    system_prompt = (
        ...
        "'摘要表/释义/项目总体情况'、以及本章的写作要求 SKILL.md。"
        "你的任务是严格按 SKILL.md 的结构，优先用'已保存的摘要表/释义/项目总体情况'里的真实值"
    ...
    system_prompt += (
        "\n\n你还能**联网搜索**（$web_search）。当某项公开信息在“已保存摘要/释义/项目总体情况”、天眼查、"
    ...
    user_prompt = (
        f"# 全局总纲 planning.md\n\n{planning}\n\n"
        f"# 已保存的摘要表/释义/项目总体情况（用户已核对，优先以此为准）\n\n{saved_summary}\n\n"
    ...
    if isinstance(data, dict) and data.get("sections"):
        ...
        # 确定性展开表格合并记号（^^ 向上 / << 向左）→ colspan/rowspan，使编辑区直接显示合并
        _expand_all_grid_merges(data["sections"])
    try:
        _save_json(n, data)
    except Exception as e:
