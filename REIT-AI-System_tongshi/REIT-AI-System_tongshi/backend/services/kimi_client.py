...
_OCR_BATCH = 4            # 视觉识别每批最多几张图（防止一次塞太多图过载/超时）
...
def ocr_images(images: list, model: str = None, instruction: str = "") -> str:
    ...
    # 分批发送：一次塞太多图会超时/过载，按每批 _OCR_BATCH 张切开、分多次识别再拼接
    use_model = model or MOONSHOT_VISION_MODEL or MOONSHOT_MODEL  # 默认用主模型（kimi-k3 支持读图）
    parts = []
    for start in range(0, len(images), _OCR_BATCH):
        batch = images[start:start + _OCR_BATCH]
        content = [{"type": "text", "text": prompt}]
        for img in batch:
            b64 = base64.b64encode(img).decode("ascii")
            content.append({"type": "image_url",
                            "image_url": {"url": f"data:image/png;base64,{b64}"}})
        resp = _create(client, model=use_model,
...
def chat_with_tools(messages: list[dict], tools: list, tool_executor,
                    ...
                    max_rounds: int = 16, should_cancel=None) -> str:
    ...
    should_cancel: 可选的无参回调，返回 True 时在"轮与轮之间""工具与工具之间"就地中止（抛 GenerationCancelled）。
    """
    def _ck():
    ...
    for i in range(max_rounds):
        ...
        # 逐个执行并回喂结果
        for tc in tool_calls:
            _ck()  # 每个工具（含耗时的 OCR）执行前检查取消，尽快停下
