...
def load_generated(pid: str, n: int) -> list:
    """读 ch{n}.json，展平为 [{title, text}]；未生成时抛 FileNotFoundError。"""
    ...
    for s in secs or []:
        ...
        out.append({"title": str(s.get("title", "")).strip(), "text": text})
    return out
...
_SCORE_SYSTEM = (
    ...
    "请按以下维度给生成稿打分（各维度0-100，权重见括号）：\n"
    "1. 内容覆盖度（30%）：标准答案中的小节、要点、表格在生成稿中是否齐全，缺失要点要在 missing_points 列出；\n"
    "2. 关键要素一致性（30%）：名称、数字、日期、文号、证照等关键信息与标准答案是否一致，有无编造或错位；\n"
    ...
    '"total":0,"summary":"","missing_points":["..."]}')
...
def score_chapter(pid: str, n: int) -> dict:
    ...
    user_prompt = (
        f"以下是第{n}章的逐节对照。请严格按系统要求输出评分JSON。\n\n" + "\n".join(parts))
