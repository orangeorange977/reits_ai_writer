...
function renderStepper(container, chapters) {
    ...
    const html = chapters.map((ch, index) => {
        ...
        // 真实章号：优先用 chapterNum（摘要表=0，正文一~七=1~7），否则退回 DOM 序号
        const chapterNum = (ch.chapterNum != null) ? ch.chapterNum : (index + 1);
        // 圆圈内容：优先用自定义 circle；否则 done 显示✓、其余显示章号
        let circleContent = ch.circle != null
            ? ch.circle
            : (ch.status === 'done' ? '✓' : chapterNum);
        // 点击动作：优先用自定义 onClick；否则默认按真实章号
        const onClick = ch.onClick || `selectChapter(${chapterNum})`;

        return `
            <div class="step ${ch.status}" data-chapter="${chapterNum}" onclick="${onClick}">
